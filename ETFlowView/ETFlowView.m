...
- (CGRect)normalizeOriginAtFrame:(CGRect)frame forSuperView:(UIView *)superview {
    ...
    if (!_isMatrix) frame.origin.y = MIN(frame.origin.y, superview.frame.size.height);
    return frame;
}
    ...
    if ([masterView.superview.subviews count] > 0) {
        ...
        for (UIView *view in masterView.superview.subviews) {
            ...
            if (_isMatrix && !(view.alpha == 0.0f) && !(view.frame.size.height <= 0.0f)) {
                CGRect normalized = [self normalizeOriginAtFrame:view.frame forSuperView:view.superview];
                preHighestY = MAX(preHighestY, normalized.origin.y);
            }
        }
        ...
        if (_isMatrix) {
            ...
            for (UIView *view in masterView.superview.subviews) {
                ...
                CGRect normalized = [self normalizeOriginAtFrame:view.frame forSuperView:view.superview];
                postHighestY = MAX(postHighestY, normalized.origin.y);
            }
